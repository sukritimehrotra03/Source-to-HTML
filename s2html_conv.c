...
void html_begin(FILE *dest_fp, int type) /* type => not used, but can be used to add differnet HTML tags */
{
	...
	fprintf(dest_fp, "</head>\n");
	fprintf(dest_fp, "<body style=\"background-color:black; font-family:'sans-serif'; font-size:16px;\">\n");
...
void source_to_html(FILE *fp, pevent_t *event)
{
    ...
    case PEVENT_HEADER_FILE:
        if (event->property == STD_HEADER_FILE)
    ...
    case PEVENT_REGULAR_EXP:
        for (int i = 0; i < event->length; ++i)
        {
            ...
            if (is_operator(ch))
                fprintf(fp, "<span class=\"operator\">%c</span>", ch);
            else if (is_symbol(ch))
                fprintf(fp, "<span class=\"symbol\">%c</span>", ch);
            else
                fputc(ch, fp);
        }
        break;
    ...
    case PEVENT_EOF:
        ...
        break;
	case PEVENT_IDENTIFIER:
		fprintf(fp, "<span class=\"identifier\">%s</span>", event->data);
		break;

    default:
        fprintf(fp, "%s", event->data); // fallback for any unknown type
        break;
    }
}